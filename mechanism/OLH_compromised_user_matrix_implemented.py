...
def olh(ori_samples, eps, domain, r, beta, post_type):
    ...
    if beta > 0:
        ...
        compromised_perturb_vector = np.random.randint(0, g-1, size=(fake_user_num, domain))
        # fake_index = np.random.randint(g)
        fake_index = g
        compromised_perturb_vector[:, target_item] = fake_index
        perturb_matrix[-fake_user_num:] = compromised_perturb_vector
        projected_index[-fake_user_num:] = fake_index
