...
def sw(ori_samples, l, h, eps, target_value, fake_user_num, attack_type, randomized_bins=1024, domain_bins=1024):
    ee = np.exp(eps)
    ...
    # fake value
    if np.size(target_value) != 0 and fake_user_num > 0:
        bin_width = (1+w) / randomized_bins
        print("eps:", eps)
        if attack_type == 0:
            print("attack on one bin")
            if target_value[0] == 1:  # maximizing mean
                # fake_value = np.random.uniform(1+w/2-bin_width, 1+w/2, size=fake_user_num)
                fake_value = np.ones(fake_user_num) * (1 + w/2)
            if target_value[0] == 0:   # minimizing mean
                # fake_value = np.random.uniform(-w/2, -w/2 + bin_width, size=fake_user_num)
                fake_value = np.ones(fake_user_num) * (-w/2)

        # if attack_type == 1:
        #     if target_value[0] == 1:  # maximizing mean
        #         print("attack on [1+b/2, 1+b]")
        #         for i in range(len(target_value)):
        #             fake_value = np.random.uniform(target_value[i] + w/4, target_value[i] + w/2, size=fake_user_num)
        #     if target_value[0] == 0:  # minimizing mean
        #         print("attack on [-b, -b/2]")
        #         for i in range(len(target_value)):
        #             fake_value = np.random.uniform(target_value[i] - w/2, target_value[i] - w/4, size=fake_user_num)

        if attack_type == 1:
            if target_value[0] == 1:  # maximizing mean
                print("attack on [1+2b/3, 1+b]")
                for i in range(len(target_value)):
                    fake_value = np.random.uniform(target_value[i] + w/3, target_value[i] + w/2, size=fake_user_num)
            if target_value[0] == 0:  # minimizing mean
                print("attack on [-b, -2b/3]")
                for i in range(len(target_value)):
                    fake_value = np.random.uniform(target_value[i] - w/2, target_value[i] - w/3, size=fake_user_num)

        if attack_type == 2:
            if target_value[0] == 1:  # maximizing mean
                print("attack on [1, 1+b]")
                for i in range(len(target_value)):
                    fake_value = np.random.uniform(target_value[i], target_value[i] + w/2, size=fake_user_num)
            if target_value[0] == 0:  # minimizing mean
                print("attack on [-b, 0]")
                for i in range(len(target_value)):
                    fake_value = np.random.uniform(target_value[i] - w/2, target_value[i], size=fake_user_num)

        if attack_type == 3:
            if target_value[0] == 1:  # maximizing mean
                print("attack on [1-b, 1+b]")
                for i in range(len(target_value)):
                    fake_value = np.random.uniform(target_value[i] - w/2, target_value[i] + w/2, size=fake_user_num)
            if target_value[0] == 0:  # minimizing mean
                print("attack on [-b, b]")
                for i in range(len(target_value)):
                    fake_value = np.random.uniform(target_value[i] - w/2, target_value[i] + w/2, size=fake_user_num)

        noisy_samples = np.concatenate( (noisy_samples[:-np.size(fake_value)], fake_value) )
    ...
    # bin_index = np.linspace(-w/2, 1+w/2, randomized_bins + 1)[1:] - 1 / (2 * randomized_bins)
    # plt.bar(bin_index, ns_hist / len(noisy_samples), width=(1+w) / randomized_bins)
    # plt.show()

    return transform, noisy_samples, ns_hist
    # return EM(n, ns_hist, transform, max_iteration, loglikelihood_threshold) * len(noisy_samples), EMS(n, ns_hist, transform, max_iteration, loglikelihood_threshold) * len(noisy_samples)
